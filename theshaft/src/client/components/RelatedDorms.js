...
export default class RelatedDorms extends Component {
    ...
    render() {
        ...
        var relatedDormsList = this.state.relatedDorms.map((dorm, index) => (<Link key={index} to={{pathname : "/explore/" + dorm.id.replace(/\s+/g, ''), dorm : dorm.DORM}} style={{ textDecoration: 'none' }}><img src={dorm.image} className="relatedDormImage" alt={dorm.name}/><div className="relatedDormName"> {dorm.name} </div></Link>));
        return (
