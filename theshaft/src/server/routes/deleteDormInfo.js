...
function deleteDormInfo(con, dormName, tableName, callback) {
	con.connect(function(err) {
	  if (err) throw err;
	  console.log("Connected!");

	  var sqlStatement = `DELETE * FROM \`${tableName}\` WHERE dorm = "${dormName}"`
...
router.post('/', function(req, res, next) {
	...
	deleteDormInfo(con, dormName, tableName, (dormInfo) => {
		res.json(dormInfo)
	})
