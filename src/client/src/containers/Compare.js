...
const options = [
    '47Claremont', 'EastCampus', '600W113th', 'RiverHall'
  ];
...
export default class Compare extends React.PureComponent {
  constructor(props) {
    ...
    this.chooseCompare = this.chooseCompare.bind(this);
  }

  chooseCompare(e) {
    var dormName = e.value;
    var dormsSelected = this.state.dormsSelected;
    dormsSelected.select1 = dormName;
    this.setState(state => ({
        dormsSelected: dormsSelected
      }));
  ...
  componentDidMount() {
    ...
    var dormName = this.state.dormsSelected.select1;
    this.fetchDormInfo(dormName);
    this.fetchAmenities(dormName);
    this.fetchReviews(dormName);
    this.fetchRelatedArticles(dormName);
  ...
  render() {
    ...
    let exploreMore = "../explore/" + this.state.dormsSelected.select1;
    return (
        
      <ScrollToTop>
          <Drop>
            <Dropdown options={options} onChange={this.chooseCompare} value={this.state.dormsSelected.select1} placeholder="Select an option" />
        </Drop> 
